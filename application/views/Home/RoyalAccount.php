...
<html class="no-js" lang="zxx">
    ...
    <body>  
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-sm-12 py-5 d-flex flex-column align-items-center" style="background-color: #ffe3ee;">
					<h2 class="text-center head-title" style="color:#00325f;font-family:Montserrat-Bold, sans-serif;">SLICK PATTERN | ROYAL CLUB</h2>
					<div class="row my-3">
						<div class="col-sm-12">
							<div class="bg-white  d-flex align-items-center main-box">
								<div class="profile-box mx-3">
									<div class="img-container">
										<?php
											if(!empty($user->image))
											{
											?>
											<img src="<?= base_url('uploads/profile_pic/').$user->image?>" alt= "avatar"  class="profile-img">
											<?php
											}
											else
											{
											?>
											<img src="https://cdn-icons-png.flaticon.com/512/219/219986.png" alt="avatar" class="profile-img">
											<?php	
											}
										?>
									</div>
								...
								<?php 
									$alertDuration=((int)($subscriber->plan_duration)-1); 
									$end_date=date('jS M Y',strtotime("+".$subscriber->plan_duration." months",strtotime($subscriber->created_at))); 
									$expire_date=date('y-m-d',strtotime("+".$subscriber->plan_duration." months",strtotime($subscriber->created_at))); 
									$current_date=date('y-m-d')	; 
									$diff =  date_diff(date_create($current_date),date_create($expire_date))->format("%R%a");
								?> 
								<div class="Subscription-details mx-2">
									<p class="mb-0 font-weight-bold" style="font-size:18px;">Hello,<?= $user->name?></p> 
									<p class="mb-0" style="font-weight:400;"><?php if($diff<=0){?><?=$subscriber->plan_duration?> Months Membership Has Been Expired On <b><?=$end_date?></b><?php }else{ ?><?=$subscriber->plan_duration?> Months Membership Expiring on <b><?=$end_date?></b><?php } ?></p>
								</div>
								<div class="Start-rating">
									<img src="https://cdn.fcglcdn.com/brainbees/images/d3monthsStrip.png">
								</div>   
							</div>
						</div>
					</div>
			...
			<div class="row jumbotron my-0">  
				<div class="col-sm-12">
					<h2 class="text-center head-title" style="color:#00325f;font-family:Montserrat-Bold, sans-serif;">CONTINUE TO ENJOY CLUB BENIFITS</h2>
							...
							if(!empty($royalcard)){ 
								...
								<div class="col-sm-3 col-7 my-2 mx-1 three_monthSub-block text-center" > 
									<div class="three_monthSub-label">
										<div class="<?php if($sr%2==0){echo 'premium_label';}else{echo 'basic_label';}?>"><?php if($subscriber->plan_duration==$card->duration){echo 'RENEW';}else{echo 'UPGRADE TO';}?></div>
									</div>
									...
									<?php } ?>
								<?php if($card->duration<=3){?>
								<span style="color: grey;">With Limited Benifits</span>
								<?php }else{?>
								<span style="color: grey;">With All Benifits</span>   
								<?php } ?>
								...
								<span class="B20_white" style="font-size:16px;" ><?php if($subscriber->plan_duration==$card->duration){echo 'RENEW NOW';}else{echo 'UPGRADE TO';}?></span>
								...
								<h2 class="text-center head-title" style="color:white;font-family:Montserrat-Bold, sans-serif;">SAVING DETAILS</h2><br>
								</div>
								<div class="container pb-5">
								<div class="row m-0 bg-white" style="border-radius:7px;">
								<div class="col-sm-12 bg-gray py-3" style="border-radius:7px;"><p class="mb-0 text-center">Savings so far with your Club membership: 166.12</p></div>
								<div class="col-sm-12">
								<div class="row">
								<div class="col-sm-6">
								<table class="saving-details-table table">
								<tr><td class="d-flex justify-content-between"><span>Product Discounts:</span><span>₹17.12</span></td></tr>
								<tr><td class="d-flex justify-content-between"><span>Shipping Cost:</span><span>₹17.12</span></td></tr>
								<tr><td class="d-flex justify-content-between"><span>Slickpattern App Subscription:</span><span>₹17.12</span></td></tr> 
								</table>
								</div>
								<div class="col-sm-6">
								<table class="saving-details-table table">
								<tr><td class="d-flex justify-content-between"><span>Club Cash Earned on Products:</span><span>₹17.12</span></td></tr>
								<tr><td class="d-flex justify-content-between"><span>Bank Offer discounts:</span><span>₹17.12</span></td></tr>
								</table>
								</div>
								</div>
								</div>
