...
<head>
    <meta charset="UTF-8">
    <title>Slick Pattern - Become a seller</title>
...
<body>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6 mx-auto">
                    ...
                    <div class="row">
                        <?php
                        $list = $this->db->get("tbl_stayconnecttop")->result();
                        foreach ($list as $item) {

                        ?>

                            <div class="col-sm-4 col-4 ">
                                <center><img alt="<?= $item->alt; ?>" title="<?= $item->seo_title; ?>" src="<?= $this->data->lazyLoader; ?>" style="max-height: 140px;" data-src="<?= base_url('./uploads/brand/'.$item->image)?>" class="img-fluid lazy">
                                    <span><?= $item->name; ?></span>
                                </center>
                            </div>
                        <?php
    ...
    <div class="col-12 col-sm-5 col-lg-5 mx-auto">
        <h2 class="mt-3 text-center pageheading ">Become a seller</h2>
        <form action="<?php echo base_url($this->data->controller . '/' . $this->data->method . '/Add'); ?>" method="post" enctype="multipart/form-data" id="addForm">
            <div class="row">
                <div class="col-lg-12 col-12">
                    ...
                    <div class="form-group">
                        <label>Address</label>
                        <textarea type="text" name="address" class="form-control form-control-lg" placeholder="Address"> </textarea>
                    </div>
    ...
    <section class="pro-content login-content">
        <div class="container">
            <div class="row">
                <?php
                $list = $this->db->get("tbl_becomeseller")->result();
                if (!empty($list)) {
                    foreach ($list as $item) {
                ?>
                        <div class="col-sm-4 p-2">
                            <div class="card" style="background-color: #f8f9fa">
                                <div class="card-header" style="background-color: #f8f9fa">
                                    <center><img alt="<?= $item->alt; ?>" title="<?= $item->seo_title; ?>" src="<?= $this->data->lazyLoader; ?>" data-src="<?= base_url('./uploads/brand/'.$item->image1);?>" class="img-fluid lazy" style="height: 100px"></center>
                                    <hr>
                                </div>
