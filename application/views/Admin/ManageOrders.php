                ...
                <div class="content-body">
                    <section id="form-action-layouts">
                        <div class="row match-height">
							<?php if($this->uri->segment(3)=='ReturnOrders' || $this->uri->segment(3)=='RefundOrders'){?>
								<div class="col-sm-12">
									<div class="card card-dashboard">
										<div class="card-content collpase show">
											<div class="card-body table-responsive">
												<div class="">
													<h2 class="text-uppercase">Requested For <?= $this->data->subTitle;?></h2>
															...
															<?php 
																...
																if (!empty($orders)) {
																	foreach ($orders as $data) { 
																		...
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'delivered','is_status'=>'true','return_status!='=>''))->result();
																		
																		if(!empty($cartOrder)){	
																			foreach($cartOrder as $cart){
																				$totalPrice=0;
																				$giftWrapForThisProduct=0;
																				$product = $this->db->get_where('products',array('id'=>$cart->product_id));
																				if(isset($product->gift_wrap) && $product->gift_wrap=='true'){
																				...
																				$discount=(float)(((float)$coupon_discount/(float)$totalProductPrice)*(float)$total);
																				...
																				$return_type="";
																				if($this->uri->segment(3)=='ReturnOrders'){ 
																					$return_type='exchange';
																				}
																				elseif($this->uri->segment(3)=='RefundOrders'){
																					$return_type='refund';
																				}
																				if($return_details->return_type==$return_type){
																				?> 
																				<tr>
																					...
																					<td class="col-sm-12">
																						<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
																							...
																							<?php if($this->uri->segment(3)!="AllOrders"){?>
																								<div class="btn-group" role="group">
																									...
																									<ul class="dropdown-menu" aria-labelledby="dropdownMenuLink">
																										<?php if($this->uri->segment(3)!="ProcessingOrders"){?>      
																											...
																											<?php } if($this->uri->segment(3)!="CancelOrders"){?>
																											...
																											<?php } if($this->uri->segment(3)=="ReturnOrders" AND ($cart->return_status=='approved')){?>
																											...
																											<?php } if($this->uri->segment(3)=="RefundOrders" AND ($cart->return_status=='approved')){?>
																											...
																											<?php } if($this->uri->segment(3)!="LostOrders"){?>
																											<li><a class="dropdown-item" href="javascript:void(0)" onclick="UpdateOrderStatus('<?=$cart->id?>','LostOrders','')">Mark as Lost Order</a></li>    
																										<?php } ?>
																									</ul> 
																								</div>
																							<?php } ?>
																					...
																					<td>
																						<?php
																							if($cart->is_combo!='NA' && isset($product->vendor_id)){
																								$vDetails = $this->db->get_where('tbl_vendor', array('id' => $product->vendor_id))->row();
																							?>	
																							<a target="_blank" href="<?= base_url('Admin/ManageUsers/UserFullDetails/'.$uid)?>" ><?= !empty($vDetails) ? $vDetails->name :'Anshul Gupta' ?></a>
																							<?php }
																							else{ ?>
																							<a href="javascript:void(0);" >Anshul Gupta</a>
																						<?php } ?>
																					</td>
																					...
																					?>
																					<td><span class="badge px-1 cursor-pointer <?=$bg_color?>" <?php if(!empty($toltip_msg)){?>data-toggle="tooltip" data-placement="top" title="<?= $toltip_msg ?>"<?php } ?> onclick="CancelView('<?=$cart->id?>')"><?=$order_status?></span></td>
																					<?php
																						$oid =  $data['orderid'];
																					?>
																					<td><?= $data['pay_mode'] ?></td>
														...
														<?php 
															...
															if (!empty($orders)) {
																foreach ($orders as $data) { 
																	...
																	$giftWrapCharge=0;
																	$action=$this->uri->segment(3);
																	if($action=='AllOrders' || $action=='PrepaidOrders' || $action=='PostpaidOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'is_status'=>'true'))->result();
																	}
																	elseif($action=='NewOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'pending','is_status'=>'true'))->result();
																	}
																	elseif($action=='ProcessingOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'processing','is_status'=>'true'))->result();
																	}
																	elseif($action=='DispatchOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'dispatched','is_status'=>'true'))->result();
																	}
																	elseif($action=='TransitOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'transit','is_status'=>'true'))->result();
																	}
																	elseif($action=='DeliveredOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'delivered','is_status'=>'true'))->result();
																	}
																	elseif($action=='RtoOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'rto','is_status'=>'true'))->result();
																	}
																	if($action=='CancelOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'cancelled','is_status'=>'true'))->result();
																	}
																	elseif($action=='ReturnOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'returned','is_status'=>'true'))->result();
																	}
																	elseif($action=='RefundOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'refunded','is_status'=>'true'))->result();
																	}
																	elseif($action=='IncompleteOrders'){
																		$cartOrder=$this->db->get_where('tbl_cart',array('orderid'=>$data['orderid'],'order_status'=>'lost','is_status'=>'true'))->result();
																	}
