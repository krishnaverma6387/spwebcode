            ...
            <div class="content-body">
                                ...
                                <?php if (($role_type == 'subadmin' && $permission->add) || $role_type == 'admin') { ?>
                                    <div class="card-header p-1">
                                        <button class="btn btn-primary" data-toggle="modal" data-target="#AddModal">
                                            ...
                                            <button class="btn btn-danger" onclick="check('brand')">
                                                    ...
                                                    foreach ($list as $item) { ?>
                                                        ...
                                                        <td><input type="checkbox" value="<?= $item->id ?>" name="id[]" class="selectall"></td>
                                                            ...
                                                            <td>
                                                                ...
                                                                if ($item->vendor_id != '0') {
                                                                    $vendor = $this->db->get_where('tbl_vendor', ['id' => $item->vendor_id])->row();
                                                                    if (!empty($vendor)) {
                                                                ?>
                                                                        <a href="javascript:void(0);" data-bs-toggle="tooltip" data-bs-placement="right" title="Vendor Details" class="btn btn-sm btn-outline-info waves-effect waves-light" onclick="VendorModal('<?= $item->vendor_id; ?>')"><?= $vendor->name ?></i></a>
                                                                    <?php
                                                                    }
                                                                } else { ?>
                                                                    <a href="javascript:void(0);" data-bs-toggle="tooltip" data-bs-placement="right" title="Vendor Details" class="btn btn-sm btn-outline-info waves-effect waves-light">Slick Pattern</i></a>
                                                                <?php }
                                                                ?>
                                                            </td>
                                                            <td><?= $item->name; ?></td>
                            ...
                            <form action="<?php echo base_url($this->data->controller . '/' . $this->data->method . '/Add'); ?>" method="post" enctype="multipart/form-data" id="addForm">
                                <div class="modal-body p-1">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control text-capitalize" name="name" placeholder="Brand Name" required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select type="text" name="vendorupload" class="form-control" required>
                                            ...
                                            <option value="NA">Slick Pattern</option>
                                            <?php
                                            $vendorlist = $this->db->get_where('tbl_vendor', ['is_status' => 'true'])->result();
                                            if (!empty($vendorlist)) {
                                                foreach ($vendorlist as $vendor) {
                                            ?>
                                                    <option value="<?= $vendor->id ?>"><?= $vendor->name ?> &nbsp -- &nbsp <?= $vendor->shop_name ?></option>
                                            <?php
                                                }
                                            }
                                            ?>
                ...
                <div class="modal fade" id="VendorModal">
                    <div class="modal-dialog">
                        <div class="modal-content border-primary">
                            <div class="modal-header bg-primary p-1">
                                <h5 class="modal-title text-white">Vendor Details</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
        ...
        $(document).ready(function() {
            ...
            $('.table tbody').sortable({
                ...
                update: function(event, ui) {
                    ...
                    console.log(order);
                    $.ajax({
                        url: '<?= base_url($this->data->controller . '/' . $this->data->method . '/SortingBrand') ?>',
                        method: 'POST',
                        data: {
                            order: order
                        },
