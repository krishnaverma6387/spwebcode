                                ...
                                <div class="card card-dashboard">
									<?php if(($role_type=='subadmin' && $permission->add) || $role_type=='admin'){?>
										<div class="card-header p-1">
											...
											<a href="<?= base_url('uploads/excel/DemoExcelSheetExpert.xlsx');?>" class="btn btn-secondary"  > <i class="fa fa-download" aria-hidden="true"></i> Demo File</a>
										</div>
									<?php } ?>
                                                            ...
                                                            <tr>
																...
																<td>
																	<?php
																		...
																		{
																		?>
																		<a href="javascript:void(0);"  class="btn btn-sm btn-outline-danger waves-effect waves-light" onclick="return Updateverification(this,'<?= $this->data->table; ?>','id','<?= $item->id; ?>','is_verified','true')"> <i class="fa fa-cross"></i> Verify</a> <br/>
																		<a href="javascript:void(0);" class="btn btn-sm btn-outline-info waves-effect waves-light" onclick="RejectExpert('<?= $item->id; ?>')"> <i class="fa fa fa-times"></i> Reject</a>
																		<?php
																		}
																		elseif($item->is_verified == 'true')
																		{
																		?>
																		<a href="javascript:void(0);" class="btn btn-sm btn-success waves-effect waves-light"> <i class="fa fa fa-check-circle"></i> Verified</a>
																		<?php
																			
																		}
																		elseif($item->is_verified == 'rejected')
																		{
																		?>
																		<a href="javascript:void(0);" class="btn btn-sm btn-danger waves-effect waves-light"> <i class="fa fa fa-times-circle"></i> Rejected</a>
																		<?php
																...
																<td>
																	<a href="<?= base_url($this->data->controller.'/'.$this->data->method.'/ExpertFullDetails/'.$item->id)?>"  class="btn btn-sm btn-secondary waves-effect waves-light" > <i class="fa fa-user"></i> View Details</a>
																</td>
																<td><?= $item->name; ?></td>
																<td><?= $item->mobile; ?></td>
																<td><?= $item->email; ?></td>
																<td><?= $item->password; ?></td>
																<td><?= $item->reject_remark; ?></td>
							...
							<form action="<?php echo base_url($this->data->controller.'/'.$this->data->method.'/Add'); ?>" method="post" enctype="multipart/form-data" id="addForm">
								<div class="modal-body p-1">
									...
									<div class="form-group">
										<label class="col-form-label">Fashion Expert Name<span class="text-danger">*</span></label>
									...
									<input type="text" data-parsley-minlength="8" data-parsley-errors-container=".errorspannewpassinput" data-parsley-required-message="Please enter your new password." data-parsley-uppercase="1" data-parsley-lowercase="1" data-parsley-number="1" data-parsley-special="1" data-parsley-required class="form-control " name="password" placeholder="Enter Password" required >
									...
									<div class="modal-content border-primary">
									<div class="modal-header bg-primary p-1">
									<h5 class="modal-title text-white">Reject <?=$this->data->key;?></h5>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
									</button>
									</div>
									
									<form action="<?php echo base_url($this->data->controller.'/'.$this->data->method.'/RejectExpert'); ?>" method="post" enctype="multipart/form-data" id="updateForm2">
									<div class="modal-body p-1">
									...
									<h5 class="modal-title text-white">Upload Fashion Expert</h5>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
									<span aria-hidden="true">&times;</span>
									</button>
									</div>
