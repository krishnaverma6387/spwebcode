													...
													<div class="div_tabs <?= $i == 1 ? 'd-block' : 'd-none' ?>" id="Tab_<?= $i ?>">
														...
														<table class="table table-striped table-bordered" style="width:100%;">
															...
															<tbody>
																...
																foreach ($item['categorytags'] as $catTag) {

																?>
																	<tr data-id="<?= $catTag['id']; ?>">
																		<td><input type="checkbox" value="<?= $catTag['id'] ?>" name="id[]" class="selectall selectall<?= $i ?>"></td>
																		<td><?= $srno; ?></td>
																		<?php if (($role_type == 'subadmin' && $permission->approval) || $role_type == 'admin') { ?>
																			<td>
																				<div class="custom-control custom-switch custom-control-inline">
																					<input type="checkbox" class="custom-control-input" onchange="return Status(this,'<?= $this->data->table; ?>','id','<?= $catTag['id']; ?>','is_status')" <?php if ($catTag['is_status'] == 'true') {
																																																												echo 'checked';
																																																											} ?> id="switch-id<?= $srno; ?>">
																			...
																			<td>
																				<div class="btn-group">
																					<?php if (($role_type == 'subadmin' && $permission->edit) || $role_type == 'admin') { ?>
																						<a href="javascript:void(0);" class="btn btn-sm btn-outline-info waves-effect waves-light" onclick="Edit('<?= $catTag['id']; ?>')"> <i class="fa fa fa-edit"></i></a>
																					<?php } ?>
																					<?php if (($role_type == 'subadmin' && $permission->delete) || $role_type == 'admin') { ?>
																						<a href="javascript:void(0);" class="btn btn-sm btn-outline-danger waves-effect waves-light" onclick="return Delete(this,'<?= $this->data->table; ?>','id','<?= $catTag['id']; ?>','','')"> <i class="fa fa-trash"></i> </a>
																					<?php } ?>
																				</div>
																			</td>
																		<?php } ?>
		...
		$(document).ready(function() {
			...
			$('.table tbody').sortable({
				...
				update: function(event, ui) {
					...
					$.ajax({
						url: '<?= base_url($this->data->controller . '/' . $this->data->method . '/SortingCategoryTags') ?>',
						method: 'POST',
						data: {
							order: order
						},
